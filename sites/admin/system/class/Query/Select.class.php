...
class Select
{
	...
	{
		$this->where[] = call_user_func_array('\DB::prepareQuery', func_get_args());
		return $this;
	}
	...
	{
		$this->having[] = call_user_func_array('\DB::prepareQuery', func_get_args());
		return $this;
	}

	public function addOrder($field, $isDescending = false, $isCollating = false)
	{
		$this->order[] = \DB::prepareQuery(sprintf($isCollating ? '%s COLLATE || %s' : '%s %s', $field, $isDescending ? 'DESC' : 'ASC'));
		return $this;
	}
